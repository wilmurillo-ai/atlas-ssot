#!/usr/bin/env python3
"""Fetch design context for all heading text nodes from Figma via MCP."""
import json
import urllib.request
import threading
import time

def query_heading_contexts():
    ...
    # From metadata scan, these are the ACTUAL TEXT nodes for each heading level:
    # Page heading:  1:1899 (label: 1:1902)
    # Heading 1:     1:1892 (label: 1:1927)
    # Heading 2:     1:1893 (label: 1:1930)
    # Heading 3:     1:1894 (label: 1:1931)
    # Heading 4:     1:1895 (label: 1:1932)
    # Heading 5:     1:1935 (label: 1:1936)
    # Paragraph Standard: 1:1896 (label: 1:1955)
    # Paragraph Large:   1:1897 (label: 1:1962)
    # Paragraph Small:   1:1898 (label: 1:1963)
    ...
    nodes_to_query = [
        ...
        ("heading-4", "1:1895"),
        ("heading-5", "1:1935"),
        ("paragraph-standard", "1:1896"),
        ("paragraph-large", "1:1897"),
        ("paragraph-small", "1:1898"),
    ]

    req_id = 200
    for label, node_id in nodes_to_query:
        payload = {
            "jsonrpc": "2.0",
            "id": req_id,
            "method": "tools/call",
        ...
        result = responses.get(req_id, {})
        print(f"\n===== {label.upper()} (node {node_id}) =====")
        if "result" in result and "content" in result["result"]:
            content = result["result"]["content"]
            if isinstance(content, list):
                for item in content:
                    if isinstance(item, dict) and "text" in item:
                        # Only print the first text item (the React code with CSS)
