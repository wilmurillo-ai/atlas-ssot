#!/usr/bin/env python3
"""Fetch actual Figma Local Variables via MCP get_variable_defs."""
import json
import urllib.request
import threading
import time

def query_figma_variables():
    ...
    payload = {"jsonrpc": "2.0", "id": 200, "method": "tools/call", "params": {"name": "get_variable_defs", "arguments": {"fileKey": "bVRNvc1roScaoHUlBABt9T"}}}
        ...
        if isinstance(content, list):
            for item in content:
                if isinstance(item, dict) and "text" in item:
                    print(item["text"][:10000])
        else:
            print(str(content)[:10000])
    else:
        print(json.dumps(result, indent=2)[:10000])

if __name__ == "__main__":
    query_figma_variables()
