#!/usr/bin/env python3
"""Fetch full Colors frame metadata to find ALL color text nodes."""
import json
import urllib.request
import threading
import time

def query_full_colors():
    ...
    # Query get_design_context on the full Colors frame (1:2)
    payload = {
        "jsonrpc": "2.0",
        "id": 200,
        "method": "tools/call",
        "params": {
            ...
            "arguments": {"fileKey": "bVRNvc1roScaoHUlBABt9T", "nodeId": "1:2"}
        }
    }
        ...
        if isinstance(content, list):
            for item in content:
                if isinstance(item, dict) and "text" in item:
                    # Print FULL output - we need all color values
                    print(item["text"])
        else:
            print(str(content))
    else:
